...
int baseAngleLeft = 90, shoulderAngleLeft = 90, elbowAngleLeft = 90, wristRollAngleLeft = 90, wristPitchAngleLeft = 90, gripAngleLeft = 90;
int baseAngleRight = 90, shoulderAngleRight = 90, elbowAngleRight = 90, wristRollAngleRight = 90, wristPitchAngleRight = 90, gripAngleRight = 90;
...
void processGamepadInput() {
  ...
  if (buttons & 0x0010) { moveLeft(); }     // L1 - Horizontal left
  else if (buttons & 0x0040) { moveBackward(); } // L2 - Backward
  else if (buttons & 0x0020) { moveRight(); }    // R1 - Horizontal right
  else if (buttons & 0x0080) { moveForward(); }  // R2 - Forward
  else if (buttons & 0x0050) { turnLeft(); }     // L1 + L2 - Turn left
  else if (buttons & 0x00A0) { turnRight(); }    // R1 + R2 - Turn right
  else {
    stopMovement();  // Stop the rover if no button is pressed
  }

  // Base and shoulder movement for both arms using joysticks
  baseAngleRight = constrain(baseAngleRight + map(rightX, -512, 508, -1, 1), servoMin, servoMax);
  updateServo(servoBaseRight, baseAngleRight);
  shoulderAngleRight = constrain(shoulderAngleRight + map(rightY, -512, 508, -1, 1), servoMin, servoMax);
  updateServo(servoShoulderRight, shoulderAngleRight);

  baseAngleLeft = constrain(baseAngleLeft + map(leftX, -512, 508, -1, 1), servoMin, servoMax);
  updateServo(servoBaseLeft, baseAngleLeft);
  shoulderAngleLeft = constrain(shoulderAngleLeft + map(leftY, -512, 508, -1, 1), servoMin, servoMax);
  updateServo(servoShoulderLeft, shoulderAngleLeft);

  // Right Arm Controls (prioritize button combinations)
  if ((buttons & 0x000C) == 0x000C) { // Square + Triangle for wrist pitch up
    wristPitchAngleRight = constrain(wristPitchAngleRight + 1, servoMin, servoMax);
    updateServo(servoWristPitchRight, wristPitchAngleRight);
  }
  else if ((buttons & 0x000A) == 0x000A) { // Triangle + Circle for wrist pitch down
    wristPitchAngleRight = constrain(wristPitchAngleRight - 1, servoMin, servoMax);
    updateServo(servoWristPitchRight, wristPitchAngleRight);
  }
  else if ((buttons & 0x0005) == 0x0005) { // Square + Cross for gripper open
    gripAngleRight = constrain(gripAngleRight + 1, servoMin, servoMax);
    updateServo(servoGripRight, gripAngleRight);
  }
  else if ((buttons & 0x0003) == 0x0003) { // Circle + Cross for gripper close
    gripAngleRight = constrain(gripAngleRight - 1, servoMin, servoMax);
    updateServo(servoGripRight, gripAngleRight);
  }
  // Individual button checks (only executed if no combination is active)
  else if (buttons & 0x0008) { // Triangle - Elbow up
    elbowAngleRight = constrain(elbowAngleRight + 1, servoMin, servoMax);
  ...
  else if (buttons & 0x0004) { // Square - Wrist roll anticlockwise
    wristRollAngleRight = constrain(wristRollAngleRight - 1, servoMin, servoMax);
  ...
  if ((dpad & 0x09) == 0x09) { // Up + Left for wrist pitch up
    wristPitchAngleLeft = constrain(wristPitchAngleLeft + 1, servoMin, servoMax);
    updateServo(servoWristPitchLeft, wristPitchAngleLeft);
  }
  else if ((dpad & 0x05) == 0x05) { // Up + Right for wrist pitch down
    wristPitchAngleLeft = constrain(wristPitchAngleLeft - 1, servoMin, servoMax);
    updateServo(servoWristPitchLeft, wristPitchAngleLeft);
  }
  else if ((dpad & 0x0A) == 0x0A) { // Down + Left for gripper open
    gripAngleLeft = constrain(gripAngleLeft + 1, servoMin, servoMax);
    updateServo(servoGripLeft, gripAngleLeft);
  }
  else if ((dpad & 0x06) == 0x06) { // Down + Right for gripper close
    gripAngleLeft = constrain(gripAngleLeft - 1, servoMin, servoMax);
    updateServo(servoGripLeft, gripAngleLeft);
  }
  // Individual D-pad checks
  else if (dpad & 0x01) { // Up - Elbow up
    elbowAngleLeft = constrain(elbowAngleLeft + 1, servoMin, servoMax);
    ...
    wristRollAngleLeft = constrain(wristRollAngleLeft - 1, servoMin, servoMax);
